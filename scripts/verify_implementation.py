...
def verify_system():
    print("=================================================================")
    print("  MULTI-STRATEGY ENSEMBLE (STRAT-001 & STRAT-002) VERIFICATION   ")
    print("=================================================================")

    # 1. Economic News Guardrail Check
    news_filter = EconomicNewsFilter()
    blocked, reason = news_filter.is_news_blocked()
    print(f"[1/5] Economic News Filter Status: Blocked={blocked} | Detail: {reason if blocked else 'Clear to trade'}")

    # 2. MT5 Adapter Connection Check
    adapter = MT5Adapter(symbol="XAUUSD")
    if adapter.connect():
        print(f"[2/5] MT5 Broker Adapter Connected successfully. Resolved symbol: {adapter.symbol}")
    else:
        print("[2/5] [WARNING] MT5 Broker Adapter running in offline simulation fallback mode.")

    # 3. M5 FVG Filter Check (STRAT-001)
    fvg_filter = M5FairValueGapFilter(symbol=adapter.symbol)
    fvg_status = fvg_filter.check_fvg_status()
    print(f"[3/5] STRAT-001 M5 FVG Filter Status: Active={fvg_status['is_fvg_active']} | Type={fvg_status['fvg_type']} | GapSize=${fvg_status['fvg_gap_size']:.2f}")

    # 4. M5 CHOCH/BOS Filter Check (STRAT-002)
    bos_filter = M5StructureBreakoutFilter(symbol=adapter.symbol)
    bos_status = bos_filter.check_structure_breakout()
    print(f"[4/5] STRAT-002 M5 CHOCH/BOS Filter Status: Active={bos_status['active']} | Type={bos_status['bos_type']} | BreakoutPrice=${bos_status['breakout_price']:.2f}")

    # 5. Live Decision Engine Ensemble Payload Check
    ...
    print(f"[5/5] Live Decision Engine Payload Evaluation (ENSEMBLE Mode):")
    ...
    print("=================================================================")
    print("  VERIFICATION COMPLETE: SYSTEM IS READY FOR ENSEMBLE LIVE PAPER TRADING  ")
    print("=================================================================")
