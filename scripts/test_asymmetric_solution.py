"""
Empirical Verification of Asymmetric SL Buffer Solution.

Tests:
1. Baseline Model 2 + VP (Original SL: 3-Bar Low - 0.50).
2. Asymmetric Solution (SL: 3-Bar Low - 0.80, TP targets anchored to un-buffered structural distance + London Open 06:00-06:30 UTC filter).
"""
...
def run_asymmetric_solution_test():
    ...
    print("================================================================================")
    print("  MODEL 2 + VP ASYMMETRIC SL BUFFER SOLUTION EMPIRICAL TEST")
    print("================================================================================")

    start_time = time.time()

    print("[1/3] Loading 5-Year XAU/USD 5-Minute Parquet Data...")
    ...
    def run_solution_backtest(sl_extra_buffer=0.0, filter_london_open=False):
        trades = []
        last_trade_bar = -10
        ...
        for i in range(50, n - 100):
            ...
            if filter_london_open and (hour == 6 and minute < 30):
                continue
            ...
            if buy_signal:
                ...
                # Structural base SL (un-buffered)
                base_sl_price = recent_3_low - 0.50
                raw_sl_pips = np.clip((entry_price - base_sl_price) / pip_size, 15.0, 80.0)

                # Add asymmetric buffer to SL execution ONLY
                sl_price = entry_price - (raw_sl_pips * pip_size) - sl_extra_buffer

                # Targets remain anchored to un-buffered structural distance
                tp1_price = entry_price + (raw_sl_pips * pip_size * 1.0)
                tp2_price = entry_price + (raw_sl_pips * pip_size * 2.0)
                tp3_price = entry_price + (raw_sl_pips * pip_size * 3.0)

            else:
                direction = "SELL"
                entry_price = low_t2
                base_sl_price = recent_3_high + 0.50
                raw_sl_pips = np.clip((base_sl_price - entry_price) / pip_size, 15.0, 80.0)
    ...
    print("[2/3] Running Baseline Model 2 + VP...")
    df_base = run_solution_backtest(sl_extra_buffer=0.0, filter_london_open=False)

    print("[3/3] Running OPTIMIZED SOLUTION Engine (+ $0.30 Asymmetric Buffer + 06:00-06:30 Filter)...")
    df_opt = run_solution_backtest(sl_extra_buffer=0.30, filter_london_open=True)
    ...
    print_metrics(df_base, "1. BASELINE MODEL 2 + VP")
    print_metrics(df_opt, "2. OPTIMIZED ASYMMETRIC SOLUTION ENGINE")

    elapsed = time.time() - start_time
    print(f"\n[DONE] Asymmetric solution test finished in {elapsed:.2f} seconds.")

if __name__ == "__main__":
    run_asymmetric_solution_test()
