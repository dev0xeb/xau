"""
Side-by-Side 3-Month Comparative Backtest (May 10 - Aug 10, 2026):
1. Unfiltered Baseline Model 2 (User's Exact Technical Blueprint)
2. Optimized Model 2 (Adding Max $3.00 EMA Extension Filter + 07:00-17:00 UTC Session Window)
"""
...
def run_model2_comparison_3m():
    ...
    df_3m_5m['date'] = df_3m_5m['timestamp'].dt.date

    closes = df_3m_5m['close'].values
    opens = df_3m_5m['open'].values
    ...
    def backtest_engine(is_optimized=False):
        trades = []
        triggered_bars = set()
        ...
        for i in range(10, n - 24):
            ...
            # M5 FVG Displacement Calculation
            ...
            # Max EMA Extension Filter (Optimized Only)
            if is_optimized:
                if bull_signal:
            ...
            if bull_signal:
                ...
                trades.append({'pnl': total_trade_pnl, 'win': win, 'sl_pips': sl_pips})
        ...
        if df_tr.empty:
            return {'Trades': 0, 'WR': 0, 'PnL': 0, 'PF': 0, 'DD': 0, 'AvgSL': 0}
        ...
        avg_sl = df_tr['sl_pips'].mean()

        return {'Trades': n_tr, 'WR': wr, 'PnL': net_pnl, 'PF': pf, 'DD': max_dd, 'AvgSL': avg_sl}

    res_baseline = backtest_engine(is_optimized=False)
    res_optimized = backtest_engine(is_optimized=True)

    elapsed = time.time() - start_t

    print("=========================================================================================")
    print(f" 3-MONTH MODEL 2 COMPARATIVE BENCHMARK (MAY 10 - AUG 10, 2026) [{elapsed:.2f}s]")
    print("=========================================================================================")
    print(f" {'METRIC PARAMETER':<30} | {'UNFILTERED BASELINE MODEL 2':<28} | {'OPTIMIZED MODEL 2'}")
    print("-" * 95)
    print(f" {'Initial Capital':<30} | ${10000.00:<27,.2f} | ${10000.00:,.2f}")
    print(f" {'Final Equity':<30} | ${10000.0 + res_baseline['PnL']:<27,.2f} | ${10000.0 + res_optimized['PnL']:,.2f}")
    print(f" {'Net Profit ($ / %)':<30} | ${res_baseline['PnL']:>+10.2f} ({(res_baseline['PnL']/100):>+6.2f}%)     | ${res_optimized['PnL']:>+10.2f} ({(res_optimized['PnL']/100):>+6.2f}%)")
    print(f" {'Executed Trades':<30} | {res_baseline['Trades']:<28d} | {res_optimized['Trades']} Trades")
    print(f" {'Win Rate (%)':<30} | {res_baseline['WR']:>6.1f}%                       | {res_optimized['WR']:>6.1f}%")
    print(f" {'Profit Factor':<30} | {res_baseline['PF']:>6.2f}                        | {res_optimized['PF']:>6.2f}")
    print(f" {'Max Drawdown (%)':<30} | -{res_baseline['DD']:>5.2f}%                       | -{res_optimized['DD']:>5.2f}%")
    print(f" {'Average SL Distance':<30} | {res_baseline['AvgSL']:>5.1f} pips                   | {res_optimized['AvgSL']:>5.1f} pips")
    print("-" * 95)

if __name__ == "__main__":
    run_model2_comparison_3m()
