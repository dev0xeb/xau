"""
Day-by-Day PnL Simulation from Last Week Monday (2026-08-03) through Today (2026-08-10) for:
1. Personal Account Engine: Baseline Model 2
2. Prop Firm Engine: Relaxed VWAP Reclaim Engine
"""
...
def run_last_week_to_today():
    ...
    # Filter dates from 2026-08-03 to 2026-08-10
    target_dates = [d for d in sorted(df_5m['date'].unique()) if d >= pd.to_datetime('2026-08-03').date() and d <= pd.to_datetime('2026-08-10').date()]
    print(f"[INFO] Target simulation dates: {target_dates}")
    ...
    def run_sim(mode="baseline"):
        trades = []
        last_trade_bar = -10

        for i in range(50, n - 100):
            if dates_5m[i] not in target_dates: continue
            hour = hours_5m[i]
    ...
    df_vwap = run_sim("relaxed_vwap")

    print("\n================================================================================")
    print(" DAY-BY-DAY PnL SUMMARY: LAST WEEK MONDAY (2026-08-03) TO TODAY (2026-08-10)")
    print("================================================================================")

    records = []
    cum_base, cum_vwap = 0.0, 0.0

    for d in target_dates:
        day_str = str(d)
        day_name = pd.to_datetime(d).strftime('%A')

        sub_base = df_base[df_base['date'] == d] if not df_base.empty else pd.DataFrame()
        sub_vwap = df_vwap[df_vwap['date'] == d] if not df_vwap.empty else pd.DataFrame()

        pnl_b = sub_base['pnl'].sum() if not sub_base.empty else 0.0
        n_b = len(sub_base)
        w_b = sub_base['is_win'].sum() if not sub_base.empty else 0
        wr_b = (w_b / n_b * 100.0) if n_b > 0 else 0.0
        cum_base += pnl_b

        pnl_v = sub_vwap['pnl'].sum() if not sub_vwap.empty else 0.0
        n_v = len(sub_vwap)
        w_v = sub_vwap['is_win'].sum() if not sub_vwap.empty else 0
        wr_v = (w_v / n_v * 100.0) if n_v > 0 else 0.0
        cum_vwap += pnl_v
    ...
    print("\n--- BASELINE MODEL 2 (PERSONAL ACCOUNT ENGINE) ---")
    for _, r in df_summary.iterrows():
        print(f"Date: {r['date']} ({r['day_name']:9s}) | Trades: {r['base_n']:2d} | Daily PnL: ${r['base_pnl']:+8.2f} | Cum PnL: ${r['base_cum']:+9.2f}")
    print(f"TOTAL PERSONAL ACCOUNT NET PnL: ${cum_base:+.2f}")

    print("\n--- RELAXED VWAP RECLAIM ENGINE (PROP FIRM ENGINE) ---")
    for _, r in df_summary.iterrows():
        print(f"Date: {r['date']} ({r['day_name']:9s}) | Trades: {r['vwap_n']:2d} | Daily PnL: ${r['vwap_pnl']:+8.2f} | Cum PnL: ${r['vwap_cum']:+9.2f}")
    print(f"TOTAL PROP FIRM NET PnL: ${cum_vwap:+.2f}")

if __name__ == "__main__":
    run_last_week_to_today()
