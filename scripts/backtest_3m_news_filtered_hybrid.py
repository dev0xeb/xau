"""
3-Month Backtest for News-Filtered Master Hybrid Confluence Engine.

Implemented Rules:
1. News Filter (Rule A): Pause trading between 13:15 UTC and 14:00 UTC ONLY on News Days (13:30 volatility spike >= $15.00).
2. Dynamic Volatility SL Buffer (Rule B):
   - High Volatility Days (Range >= $25.00): SL = $2.50
   - Normal Volatility Days (Range < $25.00): SL = $1.20 (Tight SL leverage)
3. Zero Lookahead Bias + $0.20 Spread + Sequential Bar Exit Simulation.
...
def run_3m_news_filtered_hybrid():
    ...
    for d in target_dates:
        ...
        # Detect if TODAY is a News Spike Day (check if 13:30 candle volatility range >= $15.00)
        is_news_day = False
        for idx_n in day_indices:
            if hours_5m[idx_n] == 13 and minutes_5m[idx_n] in [25, 30, 35]:
                candle_range = highs_5m[idx_n] - lows_5m[idx_n]
                if candle_range >= 15.00:
                    is_news_day = True
                    break
        ...
        for i in day_indices:
            ...
            # OVERLAP WINDOW: 12:20 UTC to 15:30 UTC
            ...
            # Measure session expansion so far
            session_expansion = abs(c_close - daily_open_price)
            is_trend_expansion = (session_expansion >= 12.00)

            # RULE B: DYNAMIC VOLATILITY SL BUFFER
            # High Volatility Day (Range so far >= $25.00) -> SL buffer = $2.50; Normal Day -> SL buffer = $1.20
            sl_buffer = 2.50 if (session_expansion >= 25.00 or is_news_day) else 1.20
    ...
    print("=" * 95)
    print(f" NEWS-FILTERED MASTER HYBRID 3-MONTH REPORT (MAY 10 - AUG 10, 2026) [{elapsed:.2f}s]")
    print("=" * 95)
    ...
    print("\n" + "-" * 95)
    print(" SAMPLE EXECUTED TRADES LOG (NEWS-FILTERED MASTER HYBRID):")
    print("-" * 95)
