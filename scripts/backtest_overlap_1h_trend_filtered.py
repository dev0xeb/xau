"""
1H Trend-Filtered London/NY Overlap Micro-Intraday Strategy Engine for Gold (XAU/USD).

Execution Rules:
1. Session Window: 12:20 - 15:30 UTC (London/NY Overlap Peak Volatility).
2. 1H Trend Filter: Only BUY when close > 1H 50 EMA; Only SELL when close < 1H 50 EMA.
3. Max 1 Trade Per Day.
4. Target: Opposing 15m Supply/Demand Zone.
5. Invalidation SL: $1.20 past liquidity sweep wick.
"""
...
def run_overlap_1h_trend_filtered():
    ...
    for d in target_dates:
        ...
        for i in range(15, n - 12):
            ...
            # Bullish Sweep + Bullish Close + 1H Bullish Alignment
            bull_sweep = htf_bull and (c_low < prev_15m_low) and (c_close > c_open)

            # Bearish Sweep + Bearish Close + 1H Bearish Alignment
            bear_sweep = htf_bear and (c_high > prev_15m_high) and (c_close < c_open)
    ...
    print("=" * 95)
    print(" 1H TREND-FILTERED OVERLAP MICRO-INTRADAY REPORT (AUG 3 - AUG 10, 2026)")
    print("=" * 95)
    ...
    print("\n" + "-" * 95)
    print(" EXECUTED TRADES LOG (PAST WEEK):")
    print("-" * 95)
