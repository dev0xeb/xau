"""
Run Optimized Model 2 (06:00 - 17:00 UTC Cutoff) on Recent Period:
From Last Week Monday (2026-08-03) to Current Moment in Dataset.
Evaluates both Prop Firm Engine & Personal Account Engine.
"""
...
def run_recent_optimized_sim():
    ...
    max_dt = df_5m['timestamp'].max()
    min_dt = df_5m['timestamp'].min()
    print(f"Dataset Range: {min_dt} to {max_dt}")

    # Determine Last Week Monday
    # Latest date in dataset
    last_date = max_dt.date()
    # Find last week Monday relative to latest date in dataset
    # If latest date is 2026-08-10 (Monday), last week Monday is 2026-08-03
    start_date = pd.to_datetime("2026-08-03").date()
    print(f"Running simulation from {start_date} to {last_date}...")
    ...
    def run_period_sim(mode="baseline", max_hour=17):
        trades = []
        last_trade_bar = -10

        for i in range(50, n):
            if dates_5m[i] < start_date: continue
            hour = hours_5m[i]
            ...
            result_str = "FULL SL (-$100.00)" if is_sl else ("TP3 HIT (+200.00)" if t3_hit else ("TP2 HIT (+100.00)" if t2_hit else "TP1 ONLY (-$33.33)"))

            trades.append({
                'date': str(dates_5m[i]),
                'day': day_names_5m[i],
                'timestamp': str(timestamps[i]),
                'time_utc': str(timestamps[i])[11:16],
                'direction': direction,
    ...
    df_prop_recent = run_period_sim("relaxed_vwap", max_hour=17)
    df_base_recent = run_period_sim("baseline", max_hour=17)

    def print_period_report(df, name):
        print(f"\n================================================================================")
        print(f" OPTIMIZED SIMULATION RESULTS FOR: {name}")
        print(f" Period: {start_date} to {last_date} (Last Week Monday to Current)")
        print(f" Parameters: 06:00-17:00 UTC Cutoff | 15.0-80.0 Pip Dynamic SL Floor")
        print(f"================================================================================")
        ...
        wins = df[df['t3_hit'] | df['t2_hit']]
        t1_only = df[df['t1_hit'] & (~df['t2_hit'])]
        sls = df[df['is_sl']]
        tot_pnl = df['pnl'].sum()

        print(f"Total Executed Trades: {tot}")
        print(f"Full / Partial Wins (TP2/TP3): {len(wins)} ({len(wins)/tot*100.0:.2f}%)")
        print(f"TP1 Only Hits (-$33.33): {len(t1_only)} ({len(t1_only)/tot*100.0:.2f}%)")
        print(f"Full Stop Losses (-$100.00): {len(sls)} ({len(sls)/tot*100.0:.2f}%)")
        print(f"Net Period PnL ($100 Risk Base): ${tot_pnl:+.2f}")

        print("\n--- Detailed Trade Log ---")
        for idx, r in df.iterrows():
            print(f"{r['date']} ({r['day'][:3]}) {r['time_utc']} UTC | {r['direction']:4s} @ ${r['entry_price']:.2f} | SL: ${r['sl_price']:.2f} ({r['sl_pips']:.1f} p) | {r['result']:18s} | PnL: ${r['pnl']:+6.2f}")

    print_period_report(df_prop_recent, "PROP FIRM ENGINE (RELAXED VWAP RECLAIM)")
    print_period_report(df_base_recent, "PERSONAL ACCOUNT ENGINE (BASELINE MODEL 2)")

if __name__ == "__main__":
    run_recent_optimized_sim()
