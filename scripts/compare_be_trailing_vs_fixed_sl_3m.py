"""
3-Month Simulation & Comparative Benchmark (May 10 - Aug 10, 2026):
...
- Max Drawdown (%)
- Count of Premature BE Stopped Trades (Trades hit BE after TP1, but then went on to hit TP2/TP3!)
"""
...
def run_be_simulation_comparison_3m():
    ...
    def backtest_engine(use_be_trailing=False):
        trades = []
        triggered_bars = set()
        premature_be_stops = 0  # Count of trades hit BE after TP1, but would have hit TP2/TP3
            ...
            if bull_signal:
                ...
                for k in range(t+1, min(t+25, n)):
                    ...
                    if not t1_hit and highs[k] >= tp1:
                        t1_hit = True
                        if use_be_trailing:
                            current_sl_t2 = entry_price  # Move SL to BE
                    ...
                    # Check SL for Ticket 2 & 3
                    if t1_hit and use_be_trailing:
                        if not t2_hit and lows[k] <= current_sl_t2:
                            t2_be_stopped = True
                        if not t3_hit and lows[k] <= current_sl_t3:
                            t3_be_stopped = True
                ...
                # Track Premature BE Stops
                if use_be_trailing and (t2_be_stopped or t3_be_stopped):
                    # Check if price later reached TP2 or TP3 anyway
                ...
                for k in range(t+1, min(t+25, n)):
                    ...
                    if t1_hit and use_be_trailing:
                        if not t2_hit and highs[k] >= current_sl_t2:
                            t2_be_stopped = True
                        if not t3_hit and highs[k] >= current_sl_t3:
                            t3_be_stopped = True
    ...
    print("=========================================================================================")
    print(f" BE TRAILING vs. FIXED SL SIMULATION (MAY 10 - AUG 10, 2026) [{elapsed:.2f}s]")
    print("=========================================================================================")
    ...
    print(f" {'Premature BE Stopped Trades':<35} | 0 Trades                  | {res_be['BE_Stops']} Trades ({res_be['BE_Stops']/721*100:.1f}%)")
    print("-" * 95)

if __name__ == "__main__":
    run_be_simulation_comparison_3m()
