"""
Master 5-Year Out-of-Sample Benchmark for Model 2 (2021 - 2026 / 5 Full Years Data).
...
Covers all market regimes across 5 years:
- Bull Markets (2020-2021, 2024-2026)
- Bear Markets (2022)
- High Volatility Spikes & Inflation Crises
- Low Volatility Compression Windows
"""
...
def run_model2_5year_benchmark():
    ...
    def backtest_engine(use_be_trailing=False):
        ...
        for i in range(10, n - 24):
            ...
            if bull_signal:
                ...
                if use_be_trailing and (t2_be_stopped or t3_be_stopped):
                    ...
                    if fut_high >= tp2: premature_be_stops += 1
                ...
                trades.append({'pnl': total_trade_pnl, 'win': win, 'year': years[t]})
                ...
                if use_be_trailing and (t2_be_stopped or t3_be_stopped):
                    ...
                    if fut_low <= tp2: premature_be_stops += 1
        ...
        if df_tr.empty:
            return {'Trades': 0, 'WR': 0, 'PnL': 0, 'PF': 0, 'DD': 0, 'BE_Stops': 0, 'df': df_tr}
        ...
        return {'Trades': n_tr, 'WR': wr, 'PnL': net_pnl, 'PF': pf, 'DD': max_dd, 'BE_Stops': premature_be_stops, 'df': df_tr}
    ...
    print("=========================================================================================")
    print(f" MASTER 5-YEAR MODEL 2 BENCHMARK (2021 - 2026 / 5 FULL YEARS) [{elapsed:.2f}s]")
    print("=========================================================================================")
    print(f" {'METRIC PARAMETER':<35} | {'FIXED SL (YOUR BLUEPRINT)':<25} | {'BE TRAILING AT TP1'}")
    print("-" * 95)
    print(f" {'Initial Capital':<35} | ${10000.00:<24,.2f} | ${10000.00:,.2f}")
    print(f" {'Final Equity':<35} | ${10000.0 + res_fixed['PnL']:<24,.2f} | ${10000.0 + res_be['PnL']:,.2f}")
    print(f" {'Net 5-Year Profit ($ / %)':<35} | ${res_fixed['PnL']:>+10.2f} ({(res_fixed['PnL']/100):>+6.2f}%)   | ${res_be['PnL']:>+10.2f} ({(res_be['PnL']/100):>+6.2f}%)")
    print(f" {'Total Executed Trades':<35} | {res_fixed['Trades']:<25d} | {res_be['Trades']} Trades")
    ...
    print(f" {'Premature BE Stopped Trades':<35} | 0 Trades                  | {res_be['BE_Stops']} Trades ({res_be['BE_Stops']/res_be['Trades']*100:.1f}%)")
    print("-" * 95)

    print("\n YEAR-BY-YEAR PERFORMANCE BREAKDOWN (FIXED SL):")
    print("-" * 95)
    df_f = res_fixed['df']
    for yr in sorted(df_f['year'].unique()):
        sub_y = df_f[df_f['year'] == yr]
        y_n = len(sub_y)
        y_w = len(sub_y[sub_y['pnl'] > 0])
        y_wr = (y_w / y_n) * 100.0
        y_gp = sub_y[sub_y['pnl'] > 0]['pnl'].sum()
        y_gl = abs(sub_y[sub_y['pnl'] < 0]['pnl'].sum())
        y_pf = (y_gp / y_gl) if y_gl > 0 else y_gp
        y_pnl = sub_y['pnl'].sum()
        print(f"  Year {yr} | Trades: {y_n:4d} | Win Rate: {y_wr:5.1f}% | Net PnL: ${y_pnl:>+10.2f} | PF: {y_pf:5.2f}")

if __name__ == "__main__":
    run_model2_5year_benchmark()
