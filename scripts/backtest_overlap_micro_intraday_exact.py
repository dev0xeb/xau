"""
Exact London/NY Overlap Micro-Intraday Strategy Engine for Gold (XAU/USD).

Fixes the direction selection bug (evaluates both Bullish and Bearish 5m FVG displacement magnitude).
...
def run_overlap_micro_exact():
    ...
    for d in target_dates:
        ...
        for i in range(15, n - 12):
            ...
            # Measure 5m FVG Displacement Gap size
            bull_gap = (lows_5m[i] - highs_5m[i-2]) if (lows_5m[i] > highs_5m[i-2]) else 0.0
            bear_gap = (lows_5m[i-2] - highs_5m[i]) if (highs_5m[i] < lows_5m[i-2]) else 0.0

            bull_sweep = (c_low < prev_15m_low)
            bear_sweep = (c_high > prev_15m_high)

            # Bullish Entry: Bullish Sweep OR Strong Bullish Gap (> Bearish Gap)
            if (bull_sweep or bull_gap >= 1.00) and (bull_gap >= bear_gap):
                sl = c_low - 1.20
            ...
            # Bearish Entry: Bearish Sweep OR Strong Bearish Gap (> Bullish Gap)
            elif (bear_sweep or bear_gap >= 1.00) and (bear_gap > bull_gap):
                sl = c_high + 1.20
    ...
    print("=" * 95)
    print(" EXACT OVERLAP MICRO-INTRADAY WEEKLY REPORT (AUG 3 - AUG 10, 2026)")
    print("=" * 95)
    ...
    print(f"  Total Executed Trades:    {total_trades} Trades (1 Trade / Day)")
    print(f"  Win Rate:                 {win_rate:.1f}% ({wins} Wins / {total_trades - wins} Losses)")
    print(f"  Profit Factor:            {profit_factor:.2f}")
    print(f"  Max Drawdown:             -{max_dd_pct:.2f}%")

    print("\n" + "-" * 95)
    print(" EXECUTED EXACT OVERLAP TRADES LOG:")
    print("-" * 95)
