"""
Fetch live MT5 5m Gold candles up to present moment (Aug 14, 2026) and run 
full day-by-day simulation for current week (Aug 10 - Aug 14) for both engines.
"""
...
def update_dataset_and_run_sim():
    proc_5m_path = Path("data/processed/xau_5m_5y.parquet")
    if not proc_5m_path.exists():
        print("[ERROR] 5m dataset missing!")
        return

    df_hist = pd.read_parquet(proc_5m_path)
    df_hist['timestamp'] = pd.to_datetime(df_hist['timestamp'])
    df_hist = df_hist.sort_values('timestamp').reset_index(drop=True)

    max_hist_ts = df_hist['timestamp'].max()
    print(f"[INFO] Existing parquet max timestamp: {max_hist_ts}")

    # Connect MT5 and fetch fresh candles if available
    ...
    if MT5_AVAILABLE:
        if mt5.initialize():
            ...
            gold_symbol = 'XAUUSD'
            for sym in ['XAUUSD', 'XAUUSDm', 'GOLD', 'XAUUSD.a', 'XAUUSD.ecn']:
                if sym in symbol_names:
                    gold_symbol = sym
                    break

            mt5.symbol_select(gold_symbol, True)
            print(f"[INFO] Fetching live MT5 5m candles for {gold_symbol}...")
            rates = mt5.copy_rates_from_pos(gold_symbol, mt5.TIMEFRAME_M5, 0, 3000)
            if rates is not None and len(rates) > 0:
                df_live = pd.DataFrame(rates)
                df_live['timestamp'] = pd.to_datetime(df_live['time'], unit='s', utc=True)
                if 'tick_volume' in df_live.columns:
                    df_live.rename(columns={'tick_volume': 'volume'}, inplace=True)
                cols = ['timestamp', 'open', 'high', 'low', 'close', 'volume']
                df_live = df_live[[c for c in cols if c in df_live.columns]]
                print(f"[INFO] Fetched {len(df_live)} MT5 M5 candles up to {df_live['timestamp'].max()}")
            mt5.shutdown()
        else:
            print("[WARNING] MT5 initialize failed. Using existing dataset.")

    if not df_live.empty:
        # Standardize timestamp timezone matching
        if df_hist['timestamp'].dt.tz is None:
            df_live['timestamp'] = df_live['timestamp'].dt.tz_localize(None)
        else:
            df_live['timestamp'] = df_live['timestamp'].dt.tz_convert(df_hist['timestamp'].dt.tz)

        # Merge new live candles
        df_combined = pd.concat([df_hist, df_live], ignore_index=True)
        df_combined.drop_duplicates(subset=['timestamp'], keep='last', inplace=True)
        df_combined.sort_values('timestamp', inplace=True)
        df_combined.reset_index(drop=True, inplace=True)
        df_combined.to_parquet(proc_5m_path, index=False)
        print(f"[SUCCESS] Updated dataset saved to parquet! Total rows: {len(df_combined):,}, Max timestamp: {df_combined['timestamp'].max()}")
        df = df_combined
    else:
        df = df_hist
    ...
    # Filter to current trading week (Monday Aug 10 to Friday Aug 14)
    max_date = df['date'].max()
    min_week_date = max_date - pd.Timedelta(days=max_date.weekday())

    print("=========================================================================================")
    print(f" FULL LIVE WEEK SIMULATION: MONDAY {min_week_date} TO PRESENT MOMENT ({max_date})")
    ...
    print("-----------------------------------------------------------------------------------------")
    print(" DAY-BY-DAY PNL COMPARISON TABLE (THIS ENTIRE TRADING WEEK)")
    print("-----------------------------------------------------------------------------------------")
    ...
    print("\n-----------------------------------------------------------------------------------------")
    print(" ALL INDIVIDUAL TRADE LOGS THIS WEEK (PERSONAL ENGINE - ML GATE 50%)")
    print("-----------------------------------------------------------------------------------------")
    if len(pers_week) == 0:
    ...
    print("\n-----------------------------------------------------------------------------------------")
    print(" ALL INDIVIDUAL TRADE LOGS THIS WEEK (PROP FIRM ENGINE - ML GATE 50%)")
    print("-----------------------------------------------------------------------------------------")
    if len(prop_week) == 0:
        print(" No trades executed this week.")
    else:
