"""
Day-by-Day Simulation of Personal Engine & Prop Firm Engine for Full Previous Trading Week
(Monday August 3, 2026 - Friday August 7, 2026)
Evaluated under Real Exness Live Friction (3.5 Pips Friction + Pessimistic Execution)
"""
...
def run_previous_week_simulation():
    ...
    # Target previous full completed calendar week: August 3, 2026 to August 7, 2026
    start_date = pd.to_datetime('2026-08-03').date()
    end_date = pd.to_datetime('2026-08-07').date()

    print("=========================================================================================")
    print(f" FULL PREVIOUS TRADING WEEK SIMULATION: MONDAY {start_date} TO FRIDAY {end_date}")
    print(" Evaluated under Real Exness Friction (3.5 Pips Friction + Pessimistic Sub-Bar Execution)")
    print("=========================================================================================\n")

    # Gold Price Action Summary for previous week
    df_prev_week = df[(df['date'] >= start_date) & (df['date'] <= end_date)].copy()
    week_open = df_prev_week['open'].iloc[0]
    week_high = df_prev_week['high'].max()
    week_low = df_prev_week['low'].min()
    week_close = df_prev_week['close'].iloc[-1]
    week_range = week_high - week_low

    print(f" GOLD (XAU/USD) PRICE ACTION PREVIOUS WEEK (AUG 3 - AUG 7):")
    print(f"   Week Open : ${week_open:,.2f}")
    print(f"   Week High : ${week_high:,.2f}")
    print(f"   Week Low  : ${week_low:,.2f}")
    print(f"   Week Close: ${week_close:,.2f}")
    print(f"   Week Range: ${week_range:,.2f} ({week_range*10:.1f} pips)\n")

    # Filter trades for previous week
    pers_week = df_pers_ml[(df_pers_ml['date'] >= start_date) & (df_pers_ml['date'] <= end_date)].copy()
    prop_week = df_prop_ml[(df_prop_ml['date'] >= start_date) & (df_prop_ml['date'] <= end_date)].copy()

    print("-----------------------------------------------------------------------------------------")
    print(" DAY-BY-DAY PNL COMPARISON TABLE (FULL PREVIOUS WEEK: AUG 3 - AUG 7)")
    print("-----------------------------------------------------------------------------------------")
    print("| Date | Day | Personal Engine Trades | Personal Daily PnL | Prop Engine Trades | Prop Daily PnL |")
    print("| :--- | :--- | :---: | :---: | :---: | :---: |")

    week_dates = sorted(df_prev_week['date'].unique())
    ...
    print(f"| **WEEK TOTAL** | **SUMMARY** | **{tot_pers_trades} Trades** | **${tot_pers_pnl:+,.2f}** | **{tot_prop_trades} Trades** | **${tot_prop_pnl:+,.2f}** |")

    print("\n-----------------------------------------------------------------------------------------")
    print(" INDIVIDUAL TRADE LOGS (PERSONAL ENGINE - ML GATE 50% | AUG 3 - AUG 7)")
    print("-----------------------------------------------------------------------------------------")
    if len(pers_week) == 0:
    ...
    print("\n-----------------------------------------------------------------------------------------")
    print(" INDIVIDUAL TRADE LOGS (PROP FIRM ENGINE - ML GATE 50% | AUG 3 - AUG 7)")
    print("-----------------------------------------------------------------------------------------")
    if len(prop_week) == 0:
        print(" No trades executed this week.")
    else:
