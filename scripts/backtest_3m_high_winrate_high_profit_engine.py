"""
Institutional Dual-Target Engine (3-Month Data).

Combines:
1. 65.1% High Win Rate (50% Partial TP at +$5.00 / 50 pips + BE Trailing).
2. 2.0% Risk per Trade (Safe Lot Sizing due to ultra-low -3.55% baseline drawdown).
3. Full Range Extreme Target Expansion for remaining 50% lot.

Evaluates past 3 months dataset (May 10, 2026 - Aug 10, 2026).
"""
...
def run_3m_high_winrate_high_profit():
    ...
    risk_pct = 0.02  # 2% Risk ($200 per trade)
            ...
            if bull_sig:
                ...
                if risk_dist >= 0.80:
                    ...
                    if partial_taken:
                        ...
                        trades.append({'date': str(d), 'time': times_5m[i], 'type': 'BUY', 'entry': entry_price, 'pnl_dollar': -200.0, 'win': False, 'res': 'FULL LOSS'})
                    else:
                ...
                if risk_dist >= 0.80:
                    ...
                    if partial_taken:
                        ...
                        trades.append({'date': str(d), 'time': times_5m[i], 'type': 'SELL', 'entry': entry_price, 'pnl_dollar': -200.0, 'win': False, 'res': 'FULL LOSS'})
                    else:
    ...
    print("=" * 95)
    print(f" INSTITUTIONAL DUAL-TARGET 3-MONTH REPORT (MAY 10 - AUG 10, 2026) [{elapsed:.2f}s]")
    print("=" * 95)
    ...
    print(f"  Total Executed Trades:    {total_trades} Trades (~1.0 trades / day)")
    print(f"  Win Rate:                 {win_rate:.1f}% ({wins} Wins / {total_trades - wins} Losses)")
    print(f"  Profit Factor:            {profit_factor:.2f}")
    print(f"  Max Drawdown:             -{max_dd_pct:.2f}%")

if __name__ == "__main__":
    run_3m_high_winrate_high_profit()
