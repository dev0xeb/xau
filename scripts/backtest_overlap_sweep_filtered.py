"""
London/NY Overlap Post-Sweep Micro-Intraday Engine for Gold (XAU/USD).

Requires a 15m Liquidity Sweep of London/Asian High/Low AFTER 12:15 UTC before entering.
Target: Opposing 15m Supply/Demand Zone.
"""
...
def run_overlap_sweep_filtered():
    ...
    for d in target_dates:
        ...
        for i in range(15, n - 12):
            ...
            # OVERLAP SWEEP WINDOW: 12:20 UTC to 15:30 UTC ONLY
            ...
            london_high = np.max(highs_5m[max(0, i-12):i])
            london_low = np.min(lows_5m[max(0, i-12):i])

            # Bullish Sweep: Low < london_low, Close > london_low
            bull_sweep = (c_low < london_low and c_close > london_low)
            bull_fvg = (lows_5m[i] > highs_5m[i-2]) and ((lows_5m[i] - highs_5m[i-2]) >= 1.00)

            # Bearish Sweep: High > london_high, Close < london_high
            bear_sweep = (c_high > london_high and c_close < london_high)
            bear_fvg = (highs_5m[i] < lows_5m[i-2]) and ((lows_5m[i-2] - highs_5m[i]) >= 1.00)

            if bull_sweep or (bull_fvg and c_close > london_low):
                sl = c_low - 1.20
            ...
            elif bear_sweep or (bear_fvg and c_close < london_high):
                sl = c_high + 1.20
    ...
    print("=" * 95)
    print(" POST-SWEEP OVERLAP MICRO-INTRADAY REPORT (AUG 3 - AUG 10, 2026)")
    print("=" * 95)
    ...
    print("\n" + "-" * 95)
    print(" EXECUTED POST-SWEEP TRADES LOG:")
    print("-" * 95)
