"""
Exness Real-World Friction & Pessimistic Sub-Bar Execution Simulation with ML Probability Gate.
"""
...
def run_exness_friction_simulation():
    ...
    def collect_dataset(exness_friction_pips=2.5, slippage_pips=1.0):
        records = []
        last_trade_bar = -10
        total_friction = (exness_friction_pips + slippage_pips) * pip_size
        ...
        for i in range(50, n):
            ...
            for k in range(i, min(i + 36, n)):
                ...
                if direction == "BUY":
                    ...
                    # Strict Pessimistic Sub-Bar Order Fills
                    if sl_touched:
                        exit_bar = k
                        break
    ...
    df_data = collect_dataset(exness_friction_pips=2.5, slippage_pips=1.0)
    ...
    print("\n=========================================================================================")
    print(" EXNESS LIVE STRESS TEST (OUT-OF-SAMPLE 2023 - 2026) WITH REAL FRICTION + ML GATE")
    print("=========================================================================================")
    
    for th in [0.0, 0.50, 0.60]:
        sub = df_oos if th == 0.0 else df_oos[df_oos['ml_proba'] >= th]
        tot = len(sub)
        win_rate = (sub['is_win'].sum() / tot) * 100.0
        tot_pnl = sub['pnl'].sum()
        ...
        label = "BASELINE EXNESS LIVE (No ML)" if th == 0.0 else f"EXNESS LIVE + ML GATE ({th*100:.0f}%)"
        print(f" {label:32s} | Trades: {tot:5d} | Win Rate: {win_rate:6.2f}% | PnL: ${tot_pnl:+10.2f} | PF: {pf:5.2f} | Max DD: -{max_dd:5.2f}%")

if __name__ == "__main__":
    run_exness_friction_simulation()
